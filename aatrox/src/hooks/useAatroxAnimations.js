...
/**
 * Handles Q (ground attack) animations and chaining.
 */
export function useQAnimations(actions, mixer, movementState, qCycleRef, currentQActionRef, currentIntoIdleActionRef, targetPosition) {
  // Hard-coded global speed multiplier.
  const globalSpeed = 0.5
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      // If in 'running', cancel movement so that Q overrides movement.
      if (movementState.current === 'running') {
        if (actions['aatrox_unsheath_run01.anm']) {
...
/**
 * Handles Spell3 animations.
 */
export function useSpell3Animation(actions, mixer, movementState) {
    // Hard-coded global speed multiplier.
    const globalSpeed = 0.5
    ...
    useEffect(() => {
      const handleKeyDown = (event) => {
        ...
        // Allow W if state is 'idle', 'running', 'q', 'q_hold', or 'spell'
        if (!['idle', 'q', 'q_hold', 'running', 'spell'].includes(movementState.current)) {
          console.log('W ignored because movementState is:', movementState.current)
          return
        }
        console.log('W pressed for Spell3')
  
        // Save the previous movement state so we can resume it after the spell.
        const prevState = movementState.current
  
        // Set the state to 'spell' immediately.
        movementState.current = 'spell'
  
        if (prevState === 'running') {
          // If unsheathed (running), cancel the run animation.
          if (actions['aatrox_unsheath_run01.anm']) {
            actions['aatrox_unsheath_run01.anm'].stop()
          }
          // Play the unsheathed spell sequence.
          if (actions['aatrox_spell3_unsheath.anm'] && actions['aatrox_spell3_unsheath_to_idle.anm']) {
            const spellUnsheath = actions['aatrox_spell3_unsheath.anm']
            const spellToIdle = actions['aatrox_spell3_unsheath_to_idle.anm']
            spellUnsheath.reset()
            spellUnsheath.setLoop(THREE.LoopOnce, 0)
            spellUnsheath.timeScale = 0.45 * globalSpeed
            ...
            const onSpellUnsheathFinished = (e) => {
              if (e.action === spellUnsheath) {
                ...
                spellUnsheath.stop()
                spellToIdle.reset()
                spellToIdle.setLoop(THREE.LoopOnce, 0)
                spellToIdle.fadeIn(0.2)
                spellToIdle.timeScale = 0.45 * globalSpeed
                spellToIdle.play()
                console.log('Playing aatrox_spell3_unsheath_to_idle.anm')
                const onSpellToIdleFinished = (e2) => {
                  if (e2.action === spellToIdle) {
                    console.log('Spell3 unsheath_to_idle finished')
                    mixer.removeEventListener('finished', onSpellToIdleFinished)
                    spellToIdle.stop()
                ...
                mixer.addEventListener('finished', onSpellToIdleFinished)
              }
            }
            mixer.addEventListener('finished', onSpellUnsheathFinished)
          }
        } else {
          // For any other previous state (treat as idle).
          if (actions['aatrox_spell3.anm']) {
            ...
            const onSpell3Finished = (e) => {
              if (e.action === spell3) {
                ...
                spell3.stop()
                // Resume idle regardless of previous state.
                if (actions['aatrox_idle1.anm']) {
                  const idleAction = actions['aatrox_idle1.anm']
                  idleAction.reset()
                  idleAction.setLoop(THREE.LoopRepeat, Infinity)
    ...
    }, [actions, mixer, movementState, globalSpeed])
  }
